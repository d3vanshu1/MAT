...
export default api({
  ...
  input: z.object({
    ...
    /** Document ID for globally unique claim IDs. REQUIRED — must be a canonical UUID. */
    documentId: z.string(),
  }),
  ...
  async run(ctx, { chunkIndex, totalChunks, chunk, model, documentId }) {
    ...
    // Inject stable claim IDs — throws on parse failure (fail-closed)
    const rawText = textBlock.text.trim();
    const idTaggedText = injectClaimIds(rawText, chunkIndex, documentId);
