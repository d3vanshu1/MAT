 ...
 *   claim_id      – globally unique, deterministic:
 *                   "{documentId}:{chunkIndex}:{claimIndex}" (0-based).
 *                   Generated by injectClaimIds (extraction-prompt.ts).
 *                   Resolved to source docs via ClaimOriginMap (claim-origin-map.ts).
 *                   Legacy format "c{N}-{M}" is recognized but receives no
 *                   fabricated provenance — ambiguity is surfaced, not attributed.
 */
