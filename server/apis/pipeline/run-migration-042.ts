/**
 * Migration 042 — SRI v2 schema.
 *
 * Creates six additive tables for the Social & Reputation Intelligence module:
 *
 *   1. sri_pipeline_state   — run root / orchestrator checkpoint.
 *   2. sri_entities          — extracted counterparties and principals.
 *   3. sri_claims            — extracted claims from deal documents.
 *   4. sri_research_plan     — planned web searches per entity/platform.
 *   5. sri_evidence          — web research evidence per plan item.
 *   6. sri_findings          — adjudicated findings with severity.
 *
 * Design constraints:
 *
 *   - No severity, tier, score, or priority column on sri_entities,
 *     sri_claims, or sri_research_plan. Severity lives only on sri_findings
 *     and is set by code, never by a model.
 *
 ...
 *     document_id and source_document_id are plain UUID columns, not FKs.
 *
 *   - entity_id on sri_research_plan is ON DELETE SET NULL because a
 *     non-entity plan item legitimately has none.
 *
 *   - claim_id on sri_findings is ON DELETE SET NULL because a finding
 *     may outlive the claim that triggered it.
 *
